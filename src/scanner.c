...
static bool is_at_end()  { return *scanner.curr == '\0'; }
static char peek()       { return *scanner.curr; }
static char peek_next()  { return is_at_end() ? '\0' : scanner.curr[1]; }
static bool is_digit(char c) { return c >= '0' && c <= '9'; }
static bool is_alpha(char c) {
    return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c == '_';
}

static char advance() { scanner.curr++; return scanner.curr[-1]; }

static bool match(char expected) {
    if (is_at_end() || *scanner.curr != expected) return false;
    scanner.curr++;
    return true;
}
...
Token scan_token() {
    ...
    switch (c) {
        ...
        case '&': return make_token(match('&') ? TOKEN_AND        : TOKEN_BITWISE_AND);
        case '|': return make_token(match('|') ? TOKEN_OR         : TOKEN_BITWISE_OR);
